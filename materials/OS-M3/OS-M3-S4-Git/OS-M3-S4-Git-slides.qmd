...
Maybe add a reminder before or at the slide "Creating an RStudio Project" that from here on (optionally) the homework starts -->
...
- **Git**: Version control system that tracks and manages changes to files and code [(an open source tool)]{.notes}
- **GitHub**: Cloud-based version control platform for hosting Git repositories which includes collaboration features [(a free but commercial platform owned by Microsoft)]{.notes}
...
::: {.notes}
**Presenter Notes**: Your turn: Scan the QR code and we can have a look who of you is familiar with one or more of these issues. 
...
::: {.notes}
**Presenter Notes**: First, I would like to find out more about how familiar you are with different concepts we will touch upon later. Please scan the QR code and select the starting survey when prompted. 
...
<!-- TODO: Here we could remove the "commit" arrow - not used yet.
I would find it more intuitive to have "local" on the left (because the verbal explanation starts with the local repository).
We could add a cloud symbol at the remote server.
 -->
...
**Step 3**: Next, you are asked to provide a passphrase in the terminal. This is an optional extra-layer of protection, as it protects your keys with a password. However, you will need to type that password everytime you use your SSH key, which can get annoying. In a high-security environment, add a passphrase. For typical research use cases, you can skip that step (simply press ENTER without adding a passphrase).
...
<!-- TODO: Do we need this slide? I thought the survey results are discussed directly after doing the survey (before the break) -->
...
<!-- NOTE (can be deleted): While the first part was a nice step by step guide, here suddenly many new concepts are introduced without explaining them: What is "staged"? What does "untracked" mean? What is a "commit" and why do we do it? 

The explanations are in the speaker notes, but I think they should be on the slides.
I added a separate slide introducing a key concept before doing it in practice
-->

## Getting your project under version control: Key concept
