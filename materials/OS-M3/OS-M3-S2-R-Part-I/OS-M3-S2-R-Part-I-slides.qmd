...
- Now wide range of applications: **data manipulation, visualization, interactive web applications**, **generation of full manuscripts and reports** ...
...
Use the table on the previous slide to run the following math operations directly in the Console of RStudio (bottom left pane).
...
As an example for how different data types can behave differently: Imagine you have the value 5 stored as a number and another value "5" stored as text.

If R sees 5 as a number, it knows you can do mathematics with it. So if you run 5 + 2, it will give you 7 as a result.
If R sees "5" as text, it treats it as a sequence of characters, not a quantity. Therefore if you try to run "5" + 2 it produces an error because adding numbers to text does not make sense mathematically. So, storing data as the correct data type is important, because it determines what operations you can perform on what. We will come back to this later, this is an important point.
:::
...
<span style="color:red; font-size:0.5em;">Here I think examples would be useful? Also the presenter notes feel a little thin, perhaps add?.</span>

::: {.notes}
**Presenter Notes**:  
In R, data can come in different types, and it is important to know which type you’re working with, because it affects what you can do with it, as we saw before.
...
Another important type is factors. These are used for categorical data with predefined levels — for example, categories like hair color, such as brown, blonde, or black.
...
<span style="color:red; font-size:0.5em;">Can you add the explanation of the example as a comment? Slide feels quite full?</span>
...
<span style="color:red; font-size:0.5em;">The name of the object here does not match the previous slide - should we make it consistent you think?.</span>
...
- To apply a function to multiple numbers at once, we **group them together**.
...
<span style="color:red; font-size:0.5em;">I think this could be added as an example function in the table on the next slide, also I think there is too much focus here on combining numbers, when we can actually use this function on multiple data types (behaving differently perhaps, but still)</span>
...
<span style="color:red; font-size:0.5em;">The focus here is primarily on numeric data types right Can we specify this and perhaps mention in the notes or so that functions are not only for numeric data types?</span>
...
<div style="font-size: 0.6em;">
<span style="color:red">Perhaps make this slide more conceptual about function arguments and then introduce the round function as a repetition on the next slide? This slide also feels quite full? </span>
</div>
...
<span style="color:red; font-size:0.5em;">include the slide with c() perhaps before this one? Thoughts? And could we perhaps "warn" instructors in the notes about potential issues people might run into across all practical exercises?</span>
...
<div style="font-size: 0.6em;">
<span style="color:red">From David: For now, I made a solution slide for the exercise and put it here. Later we can move it to the end of the presentation (like in the R-Part-I presentation) if you'd like. SvG: Yes, moving to the end sounds good! </span>
</div>
...
<div style="font-size: 0.6em;">
<span style="color:red">Move to the end as well? </span>
</div>
...
Practical exercise 9 solution - add at the end.
...
- [ ] Update learning goals + key terms and definitions (if necessary) - yes, see comments
