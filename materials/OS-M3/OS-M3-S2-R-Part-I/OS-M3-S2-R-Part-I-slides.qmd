...
<span style="color:red; font-size:0.5em;"> Mindful of learners perspective: Consider having a visual queue for learners that this slide is an exercise e.g. "What do you associate with the four terms below?". Currently the information is only found in the instructor notes. ingo</span>
...
**Instructor Notes**: Keep the explanation conceptual. The goal is recognition, not mastery. Avoid going into technical distinctions unless students ask. You may briefly point to examples in the image to anchor understanding.
:::
...
<span style="color:red; font-size:0.5em;">The name of the object here does not match the previous slide - should we make it consistent you think?. Beth: I was using different numbers/labels for the examples in the explanation slides vs. practical exercise slides... i'll simplify and change it a bit.     SARA COMMENT: To help with continuity, I would consider having the code snippet from slide 29 extended to show this adding of objects e.g. line 7 is # Let's call on the objects and add them together line 8 is x + y. Beth: oh i like this suggestion but i would change the comment a bit. </span>
...
<div style="font-size: 0.6em;">
<span style="color:red">Perhaps make this slide more conceptual about function arguments and then introduce the round function as a repetition on the next slide? This slide also feels quite full? Beth: agreed... done! SARA COMMENT: Current presenter notes read more like instructor notes - check and reformat. Beth: oh yes sorry that was my mistake- thanks for catching it!  </span>
</div>
...
**Presenter Notes**: Let's try making some vectors of our own using both numerical and word data types. But remember, for each vector, just use one datatype. At the end of this exercise, you should have 2 separate vectors each assigned to a different object.
...
<span style="color:red; font-size:0.5em;">Beth: I needed to add the solution in the notes otherwise the slides would not render- I got an error code that it could not find the objects since the following slides call on them i.e. they needed to be assigned in the code chronologically before they are called on.</span>
...
::: callout-note
# Look familiar?
Think of a dataframe like a spreadsheet. The top row is like the heading and the information below (within each column) is the observations under that heading. 
:::
...
<span style="color:red; font-size:0.5em;">Add to instructor notes how df can be called upon since the solutions are now at the end and what issues people may have at this point Beth: done </span>
...
<span style="color:red; font-size:0.5em;">Maybe this could be made into an exercise?. Beth: yes the exercise is on the following slide but i can emphasize the use of $ and [] more </span>
...
<span style="color:red; font-size:0.5em;">This assignment only covers parts of the topics today - perhaps extend? Beth: added a few more items in the assignment</span>
...
<span style="color:red; font-size:0.5em;"> Beth: I'm not sure if i agree with including it before the break since there is already the pre-break quiz. It's almost like having a check-in twice. I like how Wini has it at the end as a sort of check-in that all the learning goals were covered. </span>
...
<span style="color:red; font-size:0.5em;">perhaps make it more specific to elements that were covered today? And bring back the intimidation question from the beginning for a comparison? Beth: done </span>
...
- It might be better to place solutions directly after each of the 10 practical exercises. Saving them for the end forces learners to re-engage with old tasks, potentially creates time pressure at the end of the lesson, and delays the immediate feedback they might want before moving to the next topic. BETH: I like the idea of the solutions immediately following the exercise but in the instructor notes so that it does not utilize a whole other slide and takes up teaching time + how do we feel about still having all the solutions together at the end even if we include them in the instructor notes? For me i think it'll be nice to still have them at the end.
...
If you really want to keep the c() function as an introductory example, we should make sure to reference back to these slides when we get to vectors. BETH: yes i like this second suggestion. I think understanding the c() function in isolation is important. 
