...
This work was originally created by [Nicklas Hafiz](https://orcid.org/0000-0001-9035-4483). It was subsequently adapted by [Caterina Luz Sanchez Steinhagen](https://orcid.org/0009-0006-4783-1110), [Elizabeth Waterfield](https://orcid.org/0009-0006-3725-6730), [David Rieger](https://orcid.org/0009-0004-3701-3101), [Sara Lil Middleton](https://orcid.org/0000-0001-5307-8029) and [Sarah von Grebmer zu Wolfsthurn](https://orcid.org/0000-0002-6413-3895). The current work is licenced under a CC-BY-SA 4.0 [Creative Commons Attribution 4.0 SA International License](https://creativecommons.org/licenses/by-sa/4.0/deed.en). It permits unrestricted re-use, distribution, and reproduction in any medium, provided the original work is properly cited. If you remix, transform, or build upon the material, you must distribute your contributions under the same license as the original. Code snippets are dedicated to the public domain and licenced under a CC0 1.0 [Creative Commons Universal Licence](https://creativecommons.org/publicdomain/zero/1.0/).
...
**Creator**: Sanchez Steinhagen, Caterina Luz (![](images/ORCID-iD_icon_16x16.png){fig-alt="orcid logo"} [0009-0006-4783-1110](https://orcid.org/0009-0006-4783-1110))
...
| RStudio | Version 2025.05.1+513 or higher | [Download RStudio](https://posit.co/download/rstudio-desktop/) |
| R | Version 4.4.3 or higher | [Download R](https://www.r-project.org/) |
</div>
...
b. I have done it using point-and-click tools (e.g., Excel or SPSS).
c. I have done it using code (e.g., in R or Python).

::: {.notes}
**Presenter Notes**: Ask the learners about their experience with creating data visualizations, such as histograms or scatter plots. In Part II, we will cover how to create simple histograms.
:::
...
![](images/wd_session_menu.png){style="width:45%;"}
:::
...
<div style="font-size: 0.6em;">
Using the output pane (bottom right in default layout):
</div>

::: incremental
<div style="font-size: 0.6em;">
1. Navigate to your desired folder in the **output** pane.
...
::: {style="text-align: center;"}
![](images/wd_files_pane.png){style="width:50%;"}
:::
...
**Instructor Notes**: Remind students that simply navigating to a folder in the output pane does not automatically set it as the working directory. They must explicitly execute the "Set As Working Directory" command.
:::
...
- [ ] Locate the folder where you saved your R-script during Part I. 
- [ ] Ensure this folder contains only your script and relevant workshop files.
- [ ] Set this folder as your Working Directory using either Method 1 or Method 2.
- [ ] Verify that the correct path is displayed at the top of your console.
...
- Click [**here**](https://github.com/nickhaf/r_tutorial/tree/main/raw_data) to download the `characters.rds` file use the link below.

<https://github.com/nickhaf/r_tutorial/tree/main/raw_data>


::: {.notes}
**Presenter Notes**: Before we can load and inspect data, we need to have it on our computers. Please use the link or scan the QR code to download the `characters.rds` file. This dataset contains fun information about TV show characters, specifically from "Brooklyn Nine-Nine" and "Community", including variables like their show's name and a "notability" score. We will use this dataset to practice data exploration and visualization.
...
## Codebooks
<div style="font-size: 0.8em;">
Variable names are often abbreviated. It is highly advisable to use a **codebook** for precise definitions. Check out our [training material on data dictionaries](https://lmu-osc.github.io/data-documentation-validation-R/02-data-dictionaries/page1.html) to learn more (https://lmu-osc.github.io/data-documentation-validation-R/02-data-dictionaries/page1.html)!
</div>
:::
...
::: {.notes}
**Presenter Notes**: Numbers are great, but visualizing data often makes it much easier to understand. A histogram is one of the most common plots for numeric variables. It shows us how often different values occur in our dataset. We create it using the `hist()` function. When we run this code, the plot will automatically appear in the output pane in the bottom right corner of RStudio. We can also customize these plots, for example by changing the title or the axis labels, so they look clean and don't overlap. Histograms are incredibly useful because they show us the entire distribution and actual shape of the data, not just a single summary number like the mean.

**Instructor Notes**: Ensure learners know to look at the bottom-right pane for their output. If their output pane is too small, RStudio might throw a "figure margins too large" error. Tell them to simply drag the pane borders to make it larger if that happens.
:::
...
::: {.notes}
**Presenter Notes**: This is what your output should look like in the output pane. Looking at the plots, you can visually confirm what the means calculated earlier suggested: the distributions are different, and we can now actually see where the majority of characters fall on the notability scale for each universe. Here you can clearly see the side-by-side comparison of both distributions.
:::
...
::: {.notes}
**Presenter Notes**: Let's go through the solutions.
Line 1 was missing quotation marks around the file name. Line 2 used a capital M for the mean function, but R functions are lowercase. Line 3 was missing quotation marks around the show name AND was missing the crucial comma inside the brackets.
:::
...
- **Wrangle and transform:** Subsetting, cleaning, and reshaping data to fit your needs. *(We did this!)*
...
::: incremental
<div style="font-size: 0.8em;">
- **Advanced visualizations:** Packages like [`ggplot2`](https://ggplot2.tidyverse.org/) (https://ggplot2.tidyverse.org/) allow you to create stunning, highly customized graphics.
- **Interactivity:** Packages like [`shiny`](https://shiny.posit.co/) (https://shiny.posit.co/) let you build interactive web applications for your data.
- **Reproducibility:** By scripting your analysis, anyone (including future you) can re-run your exact steps with zero guesswork.
</div>
:::

::: {.notes}
**Presenter Notes**: As you continue learning, you will discover that R is incredibly powerful. You can move from simple base R histograms to stunning, publication-ready graphics using packages like ggplot2. You can even build interactive websites using packages like shiny. If you're curious about either of these packages, both ggplot2 and shiny are clickable links on this slide that will take you straight to their official websites. Most importantly, by writing scripts instead of clicking through menus, your research becomes 100% reproducible. 
:::
...
<span style="color:red;">SvG: I am wondering if we would need an extra slide to perhaps tell learners about how to use AI responsibly and also specific tools for code and troubleshooting? This feels like it could be a slide deck in itself, and we are promoting it without giving examples and only highlighting the complexity in passing? Thoughts?</span>
<span style="color:red;">David: Since the topic of AI and tools changes so fast, a dedicated and detailed slide deck might already be outdated before it is even online. But I see your point! A single slide with some general tips and a rough overview of possibilities sounds like a good idea. I've added a draft slide right after "Final words" for you to adapt, move, or delete as you see fit.</span>
</div>
...
<div style="font-size: 0.85em;">
AI tools can significantly speed up your learning and coding process, but they require critical thinking. 
</div>

::: incremental
<div style="font-size: 0.8em;">
- **Verify everything:** AI can hallucinate or suggest outdated functions. You must still read, understand, and test the code it generates.
- **Provide good context:** When troubleshooting, provide the AI with your exact error message and the structure of your data (e.g., using `str()`).
- **Data Privacy is critical:** Never paste sensitive, personal, or unpublished research data into public AI chatbots!
- **Common Tools:** 
  - *Conversational Chatbots* (e.g., ChatGPT, Claude) are great for explaining concepts and debugging errors.
  - *Coding Environments (IDEs) with AI:* Tools integrated directly into where you write code (your IDE, like RStudio). Examples include [GitHub Copilot](https://github.com/features/copilot) (https://github.com/features/copilot) and [Cursor](https://www.cursor.com/) (https://www.cursor.com/). A major advantage is that they can "see" your entire project. This allows you to chat with the AI and have it provide highly relevant inline suggestions or directly implement code into your script, without needing to manually copy-paste context.
</div>
:::

::: {.notes}
**Presenter Notes**: As AI tools become deeply integrated into programming workflows, it's important to use them responsibly. They are excellent co-pilots for troubleshooting red text in the console or figuring out how to approach a new task. However, they are not infallible. You must understand the foundations of R to verify if the AI's code actually does what you need. When it comes to tools, you can use standalone chatbots, or integrations directly within your programming environment (also called an IDE, like RStudio). Integrations have the benefit of "seeing" your entire script, so you don't have to copy-paste as much context. Most importantly, remind learners about data privacy: public AI models often train on user inputs, so sensitive data should never be shared with them!

**Instructor Notes**: Frame AI as an assistant, not a replacement for learning the basics.
:::
...
First, correctly setting your Working Directory and loading necessary packages are the non-negotiable first steps of any R session. Without them, you cannot access your data or advanced tools.
Second, subsetting is central to data analysis. You rarely work with a full raw dataset at once; instead, you filter specific rows based on logical conditions to answer targeted questions.
And third, basic statistics and visualizations go hand in hand. Using summary functions alongside plots like histograms gives you a complete picture of your data's shape and distribution.

**Instructor Notes**:
Keep this slide concise and use it as a clear closure of the R part 2 section.
:::
...
**Instructor Notes**: Allocate about 5-10 minutes total for this think-pair-share activity (1-2 minutes for individual reflection, 3-4 minutes in pairs, and the remainder for group discussion). Facilitate an open discussion focusing on application rather than theory. Encourage concrete examples from learners’ own academic context. Actively prompt for both benefits and barriers. Keep the discussion time-bound to avoid overextension at the end of the session.
...
<div style="font-size: 0.8em;">
- Set your Working Directory and load the `characters.rds` dataset.
- Create a new subset of the data using a logical condition of your choice (e.g., a different show).
- Calculate a summary statistic (e.g., `mean()` or `table()`) on a variable within your subset.
- Create a histogram (`hist()`) to visualize a numeric variable and customize its title.
...
::: {.notes}
**Presenter Notes**:
To finish, you have a short home assignment that brings together everything we covered in Part II.
You will create a script where you first set your working directory and load the characters dataset. Then, you will practice subsetting by filtering the data based on a condition of your choosing—perhaps explore characters from a different show. Next, compute a basic summary statistic like a mean or a frequency table on your new subset. Finally, visualize a numeric variable using a histogram and give it a custom title.
Make sure to comment your code so you know exactly what each line does when you look at it later.

**Instructor Notes**:
Clarify that this is a practice exercise rather than a graded assessment. Encourage learners to explore variables in the dataset they haven't looked at yet. Reassure them that running into errors during subsetting is normal and part of the learning process.
:::
...
**1. How confident do you feel setting a Working Directory and loading external data into R?**
...
**2. How comfortable do you feel using logical conditions (`==`, `|`) to subset data frames?**
...
**3. Compared to the beginning of the workshop, how intimidated do you feel about working with R?**

a. Much more intimidated  
b. Slightly more intimidated  
c. About the same  
d. Slightly less intimidated  
e. Much less intimidated  

---

**4. What part of today's lesson still feels confusing, and which exercise helped your understanding the most?**
...
- Datacamp offers a free, [interactive course on R](https://www.datacamp.com/courses/free-introduction-to-r) online (https://www.datacamp.com/courses/free-introduction-to-r).
- The LMU Open Science Center provides a [comprehensive R tutorial](https://lmu-osc.github.io/introduction-to-R/) (https://lmu-osc.github.io/introduction-to-R/).

::: {.notes}
**Presenter Notes**:  
If you’d like to continue practicing R after this session, there are optional additional resources available.  
Datacamp offers a free, interactive introduction to R, where you can practice directly in your browser. We also highly recommend the LMU OSC's own Introduction to R tutorial, which is a great comprehensive guide. These are useful ways to reinforce what we covered today and get more comfortable with writing code at your own pace.
You are not required to complete them, but they can be helpful if you want extra practice or a more structured introduction to R outside of class.

**Instructor Notes**:  
Present these as optional extensions, not part of the core learning requirements. Avoid suggesting them as mandatory. Emphasise self-paced learning and reinforcement of basic R skills using both the interactive Datacamp course and the in-depth OSC material.

**Accessibility Tips:**  
Ensure the links are accessible through the course platform or slide deck. 
:::
